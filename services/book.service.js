...
function onAddBookGoogle(book) {
  const books = _loadFromStorage()
  books.push({
    id: book.id || 'none', title: book.volumeInfo.title || 'none', subtitle: book.volumeInfo.subtitle || 'none', authors: book.volumeInfo.authors || 'none',
    publishedDate: book.volumeInfo.publishedDate.substring(0, 4) || 'none', description: book.volumeInfo.description || 'none', pageCount: book.volumeInfo.pageCount || 'none',
    categories: book.volumeInfo.categories || [], thumbnail: book.volumeInfo.imageLinks.thumbnail || 'none', language: book.volumeInfo.language || 'none',
     listPrice: {
      amount: 0,
      currencyCode: "ILS", isOnSale: false
    }
  })
  _saveToStorage(books)
  return Promise.resolve()
